...
$filters = [];
$sql = "SELECT * FROM rings WHERE price BETWEEN :price_min AND :price_max";
...
<head>
    ...
    <title>E-COMMERCE WEBSITE - RING</title>
    ...
    <div class="page-goods-header">
      ...
      <div class="content">
        ...
        <section class="filter-section">
          <form class="filter-section-form" method="GET" action="ring.php">
          <h3 class="filter-h3">Фільтри</h3>
            ...
            <div class="btn-filter">
                <button class="resetmit-btn" type="button" onclick="resetFilters('ring.php')">ОЧИСТИТИ ФІЛЬТРИ</button>
              ...
              <div class="item-goods" data-id="<?= $product['product_id'] ?>" data-name="<?= $name ?>" data-price="<?= $price ?>$" data-description="<?= $description ?>" data-table="rings" style="background-image: url('<?= $image1 ?>');" onmouseenter="showAddGoods(this)" onmouseleave="hideAddGoods(this)">
                  <div class="good-details">
                      ...
                      <div class="add-goods hidden">
                          <div>
                              <img class="heart-icon <?= $isFavorite ? 'favorited' : '' ?>" src="image/<?= $isFavorite ? 'heart-filled-icon.png' : 'heart-icon.png' ?>" alt="Heart Icon" onclick="<?= $isFavorite ? 'removeFromFavorites' : 'addToFavorites' ?>(<?= $product['product_id'] ?>, 'rings', <?= $user_id ?>)">
                          </div>
