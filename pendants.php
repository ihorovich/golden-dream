...
$filters = [];
$sql = "SELECT * FROM pendants WHERE price BETWEEN :price_min AND :price_max";
...
$favorites = [];
if ($user_id) {
    $favoritesQuery = $db->prepare("SELECT product_id FROM favorites WHERE user_id = :user_id AND product_table = 'pendants'");
...
<head>
    ...
    <title>E-COMMERCE WEBSITE - Pendants</title>
    ...
    <div class="container">
      ...
      <div class="content">
        ...
        <section class="filter-section">
          <form class="filter-section-form" method="GET" action="pendants.php">
          <h3 class="filter-h3">Фільтри</h3>
            ...
            <div class="filter-group">
              <label>Бренд:</label>
              <div><input type="checkbox" name="brand[]" value="ROCKAH" <?= in_array('ROCKAH', $brand) ? 'checked' : '' ?>> ROCKAH</div>
              <div><input type="checkbox" name="brand[]" value="DANYLEVYCH" <?= in_array('DANYLEVYCH', $brand) ? 'checked' : '' ?>> DANYLEVYCH</div>
              <div><input type="checkbox" name="brand[]" value="MANN" <?= in_array('MANN', $brand) ? 'checked' : '' ?>> MANN</div>
              <div><input type="checkbox" name="brand[]" value="NUMERO OCTO" <?= in_array('NUMERO OCTO', $brand) ? 'checked' : '' ?>> NUMERO OCTO</div>
            ...
            <div class="filter-group">
              ...
              <div><input type="checkbox" name="color[]" value="Срібний" <?= in_array('Срібний', $color) ? 'checked' : '' ?>> Золотий</div>
              <div><input type="checkbox" name="color[]" value="Зелений" <?= in_array('Зелений', $color) ? 'checked' : '' ?>> Зелений</div>
            </div>
            <div class="filter-group">
            ...
            <div class="btn-filter">
                <button class="resetmit-btn" type="button" onclick="resetFilters('pendants.php')">ОЧИСТИТИ ФІЛЬТРИ</button>
        ...
        <section class="goods-section">
          <div class="set-of-goods-container">
            ...
            <?php else: ?>
              ...
              ?>
              <div class="item-goods" data-id="<?= $product['product_id'] ?>" data-name="<?= $name ?>" data-price="<?= $price ?>$" data-description="<?= $description ?>" data-table="pendants" style="background-image: url('<?= $image1 ?>');" onmouseenter="showAddGoods(this)" onmouseleave="hideAddGoods(this)">
                  <div class="good-details">
                      <h4 class="goods-name"><?= $name ?></h4>
                      <p class="goods-description hover-underline" onclick="redirectToProductDetailPage(<?= $product['product_id'] ?>, 'pendants')"><?= $description ?></p>
                      <p class="goods-price"><?= $price ?>$</p>
                      <div class="add-goods hidden">
                          <div>
                              <img class="heart-icon <?= $isFavorite ? 'favorited' : '' ?>" src="image/<?= $isFavorite ? 'heart-filled-icon.png' : 'heart-icon.png' ?>" alt="Heart Icon" onclick="<?= $isFavorite ? 'removeFromFavorites' : 'addToFavorites' ?>(<?= $product['product_id'] ?>, 'pendants', <?= $user_id ?>)">
                          </div>
