...
$filters = [];
$sql = "SELECT * FROM chokers WHERE price BETWEEN :price_min AND :price_max";
...
$favorites = [];
if ($user_id) {
    $favoritesQuery = $db->prepare("SELECT product_id FROM favorites WHERE user_id = :user_id AND product_table = 'chokers'");
...
<head>
    ...
    <title>E-COMMERCE WEBSITE - Chokers</title>
    ...
    <div class="container">
      ...
      <div class="content">
        ...
        <section class="filter-section">
          <form class="filter-section-form" method="GET" action="chokers.php">
          <h3 class="filter-h3">Фільтри</h3>
            ...
            <button class="resetmit-btn" type="button" onclick="resetFilters('chokers.php')">ОЧИСТИТИ ФІЛЬТРИ</button>
        ...
        <section class="set-of-goods-wrapper">
          <div class="set-of-goods-container">
            ...
            <?php else: ?>
              ...
              ?>
              <div class="item-goods" data-id="<?= $product['product_id'] ?>" data-name="<?= $name ?>" data-price="<?= $price ?>$" data-description="<?= $description ?>" data-table="chokers" style="background-image: url('<?= $image1 ?>');" onmouseenter="showAddGoods(this)" onmouseleave="hideAddGoods(this)">
                  <div class="good-details">
                      <h4 class="goods-name"><?= $name ?></h4>
                      <p class="goods-description hover-underline" onclick="redirectToProductDetailPage(<?= $product['product_id'] ?>, 'chokers')"><?= $description ?></p>
                      <p class="goods-price"><?= $price ?>$</p>
                      <div class="add-goods hidden">
                          <div>
                              <img class="heart-icon <?= $isFavorite ? 'favorited' : '' ?>" src="image/<?= $isFavorite ? 'heart-filled-icon.png' : 'heart-icon.png' ?>" alt="Heart Icon" onclick="<?= $isFavorite ? 'removeFromFavorites' : 'addToFavorites' ?>(<?= $product['product_id'] ?>, 'chokers', <?= $user_id ?>)">
                          </div>
