...
$filters = [];
$sql = "SELECT * FROM necklace WHERE price BETWEEN :price_min AND :price_max";
...
$favorites = [];
if ($user_id) {
    $favoritesQuery = $db->prepare("SELECT product_id FROM favorites WHERE user_id = :user_id AND product_table = 'necklace'");
...
<head>
    ...
    <title>E-COMMERCE WEBSITE - Necklace</title>
    ...
    <div class="container">
      ...
      <div class="content">
        ...
        <section class="filter-section">
          <form class="filter-section-form" method="GET" action="necklace.php">
          <h3 class="filter-h3">Фільтри</h3>
            ...
            <div class="filter-group">
              <label>Бренд:</label>
              <div><input type="checkbox" name="brand[]" value="ARKULYA" <?= in_array('ARKULYA', $brand) ? 'checked' : '' ?>> ARKULYA</div>
              <div><input type="checkbox" name="brand[]" value="BLACK BURO" <?= in_array('BLACK BURO', $brand) ? 'checked' : '' ?>> BLACK BURO</div>
              <div><input type="checkbox" name="brand[]" value="RIMMART" <?= in_array('RIMMART', $brand) ? 'checked' : '' ?>> RIMMART</div>
              <div><input type="checkbox" name="brand[]" value="TEPLEZH" <?= in_array('TEPLEZH', $brand) ? 'checked' : '' ?>> TEPLEZH</div>
              <div><input type="checkbox" name="brand[]" value="FREDINSKAYA JEWELRY" <?= in_array('FREDINSKAYA JEWELRY', $brand) ? 'checked' : '' ?>> FREDINSKAYA JEWELRY</div>
              <div><input type="checkbox" name="brand[]" value="SOLAR SILVER" <?= in_array('SOLAR SILVER', $brand) ? 'checked' : '' ?>> SOLAR SILVER</div>
              <div><input type="checkbox" name="brand[]" value="PURPURE QUEEN" <?= in_array('PURPURE QUEEN', $brand) ? 'checked' : '' ?>> PURPURE QUEEN</div>
            </div>
            <div class="filter-group">
              <label>Колір:</label>
            ...
            <button class="resetmit-btn" type="button" onclick="resetFilters('necklace.php')">ОЧИСТИТИ ФІЛЬТРИ</button>
        ...
        <section class="set-of-goods-wrapper">
          <div class="set-of-goods-container">
            ...
            <?php else: ?>
              ...
              ?>
              <div class="item-goods" data-id="<?= $product['product_id'] ?>" data-name="<?= $name ?>" data-price="<?= $price ?>$" data-description="<?= $description ?>" data-table="necklace" style="background-image: url('<?= $image1 ?>');" onmouseenter="showAddGoods(this)" onmouseleave="hideAddGoods(this)">
                  <div class="good-details">
                      <h4 class="goods-name"><?= $name ?></h4>
                      <p class="goods-description hover-underline" onclick="redirectToProductDetailPage(<?= $product['product_id'] ?>, 'necklace')"><?= $description ?></p>
                      <p class="goods-price"><?= $price ?>$</p>
                      <div class="add-goods hidden">
                          <div>
                              <img class="heart-icon <?= $isFavorite ? 'favorited' : '' ?>" src="image/<?= $isFavorite ? 'heart-filled-icon.png' : 'heart-icon.png' ?>" alt="Heart Icon" onclick="<?= $isFavorite ? 'removeFromFavorites' : 'addToFavorites' ?>(<?= $product['product_id'] ?>, 'necklace', <?= $user_id ?>)">
                          </div>
